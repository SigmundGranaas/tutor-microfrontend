...
import { PAGE_SIZE_COURSE } from '../constants';
...
const LMSBaseUrl = `${process.env.LMS_BASE_URL}/api/courses/v1`;
...
class DiscoveryDataApiService {
  ...
  static fetchCourses(options) {
    ...
    const queryParams = {
      page:1,
      page_size: PAGE_SIZE_COURSE,
      limit: PAGE_SIZE_COURSE,
     fields: fields.join(),
      editable: 1,
      exclude_utm: 1,
  ...
  static autocompletePerson(text, organizationKeys) {
    ...
    const url = `${discoveryBaseUrl}/search/person_typeahead/${queryString.join(
      '&org='
    )}`;
    return getAuthenticatedHttpClient().get(url);
  }
