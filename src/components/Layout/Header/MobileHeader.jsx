...
class MobileHeader extends React.Component {
    ...
    return mainMenu.map((menuItem) => {
      ...
      if (type === 'item') {
        if (hrefType === 'external') {
          return (
            <a key={`${type}-${content}`} className={active ? "nav-link active-link-responsive" : "nav-link"} href={href}>
              {content}
            </a>
          );
        }
        return (
          <Link
            key={`${type}-${content}`}
            className={active ? "nav-link active-link-responsive" : "nav-link"}
            to={href}
  ...
  renderUserMenuItems() {
    ...
    return userMenu.map(({ type, href, hrefType, content }) => (
  ...
  renderLoggedOutItems() {
    ...
    return loggedOutItems.map(({ type, href, hrefType, content }, i, arr) => (
